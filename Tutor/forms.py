...
import hashlib
import datetime
grade_choice = (('N','不限'),('Pr', '小学'), ('J1', '初一'), ('J2', '初二'), ('J3','初三'),('H1', '高一'), ('H2', '高二'), ('H3', '高三'))
subject_choice = (('Math', '数学'),('Chinese', '语文'),("English", '英语'),('Physic', "物理"), ('Chemistry', '化学'), ('Biology', '生物'))

class LoginForm(forms.Form):
    username = forms.CharField(label="",max_length=30,widget=forms.TextInput(attrs={"placeholder":"用户名"}), error_messages = {'required':"用户名不能为空"})
    password = forms.CharField(label="",widget=forms.PasswordInput(attrs={"placeholder":"密码"}), error_messages = {'required':"密码不能为空"})
    identity = forms.ChoiceField(label="",choices=((True,'家长'),(False,'家教')))
...
class EmployForm(forms.ModelForm):
    ...
    class Meta:
        model = Employ
        exclude = ['user','pub_date','is_visible']
...
class PasswordReset(forms.Form):
    ...
    password1 = forms.CharField(label="新密码",max_length=30, error_messages={
            'required':"新密码不能为空",
        },
        widget=forms.PasswordInput(attrs={"placeholder":"新密码"}))
    password2 = forms.CharField(label="重复密码",max_length=30, error_messages={
            'required':"重复密码不能为空",
        },
...
class PasswordChange(forms.Form):
    ...
    password_orgin = forms.CharField(label="原始密码",max_length=30, error_messages={
            'required':"旧密码不能为空",},
	...
	password1 = self.cleaned_data.get("password1")
        ...
        return password2
class CommentParentForm(forms.ModelForm):
    class Meta:
        model = Comment
	fields = ["content"]
class CommentTutorForm(forms.ModelForm):
    score = forms.IntegerField(max_value=5, min_value=1,required=True,error_messages={"required":"评分不能为空","invalid":"评分只能为1到5的整数"})
    class Meta:
        model = Comment
...
class searchTutorForm(forms.Form):
    ...
    subject = forms.MultipleChoiceField(label="辅导科目",choices=subject_choice,widget=forms.CheckboxSelectMultiple,required=False)
class searchAskEmployForm(forms.Form):
    pub_date = forms.ChoiceField(label="发布日期",choices=(('all','全部'),('day','最近一天'),('week','最近一周')))
    salary = forms.ChoiceField(label="薪水",choices=(('N','不限'),('0','50以下'),('1','50-100'),('2','100以上')))
    grade = forms.ChoiceField(label="辅导年级",choices=grade_choice)
    subject = forms.MultipleChoiceField(label="辅导科目",choices=subject_choice,widget=forms.CheckboxSelectMultiple(),required=False)
